...
g_omx_port_free_buffers (GOmxPort *port)
{
    ...
    /* if the queue does not contain all allocated buffers, then (omx) free
     * the ones missing
     */
    DEBUG (port, "n_free_buffers %d port->num_buffers %d", n_free_buffers,
           port->num_buffers);
    if (n_free_buffers < port->num_buffers)
    {
        for (i = 0; i < port->num_buffers; i++)
        {
            ...
            {
                DEBUG (port, "OMX_FreeBuffer(%p)", omx_buffer);
                OMX_FreeBuffer (port->core->omx_handle, port->port_index,
                                omx_buffer);
            }
        }
    }
